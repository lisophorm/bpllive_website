...
$team=$db->ExecuteSQL("SELECT teams.team_name,users.issynced FROM users INNER JOIN teams ON users.team_id = teams.id where users.urn='".$db->SecureData($_GET['urn'])."'");

if(!$team) {
	die("error quering team:".$db->lastError." - ".$db->lastQuery);
} 
...
switch($_GET['share_type']) {
	case "TWITTER":
	$incrementfield="twitter_click_trophy";
	break;
	case "FACEBOOK":
	$incrementfield="facebook_click_trophy";
	break;
	default:
	$incrementfield="email_click_trophy";
	break;
}
...
<meta property="og:description" content="I got my hands on the Barclays Premier League trophy at Barclays Premier League Live in Johannesberg, South Affrica." />
...
<div style="width:850px;margin-left:auto;margin-right:auto;background-color:#FFF;">
  <div style="width:369px;float:left;background-color:#ffffff;margin:0px;padding:0px;">
    ...
    <p style="font-size:29px;color:#1e3052">I got my hands on the Barclays Premier League trophy at Barclays Premier League Live in Johannesberg, South Affrica.</p>
    <p></p>
  </div>
